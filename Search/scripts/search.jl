...
const LOCAL_QUARTO_DOCS_INDEX = "_site/search.json"

"""
    get_docs_quarto_search_index() -> Vector{QuartoSearchEntry}
...
Note that this always fetches the search index from TuringLang/docs, not from
TuringLang/turinglang.github.io.
"""
function get_docs_quarto_search_index()
    ...
    elseif GITHUB_REPO == "TuringLang/turinglang.github.io"
        ...
        error("Unexpected GITHUB_REPOSITORY value: $GITHUB_REPO")
    end
...
# Get docs entries
all_entries = get_docs_quarto_search_index()
...
Base.rename(LOCAL_QUARTO_DOCS_INDEX, "_site/search_original.json"; force=true)
JSON.json(LOCAL_QUARTO_DOCS_INDEX, all_entries; pretty=2)
@info "Wrote $(length(all_entries)) entries to $LOCAL_QUARTO_DOCS_INDEX"
